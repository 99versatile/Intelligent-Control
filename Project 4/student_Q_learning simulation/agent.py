    ...
    def Q_learning(self, discount=1.0, alpha=0.01, max_seq_len=500,
        ...
        for episode in range(TRAINING_EPOCH_NUM):
            ...
            while not (done or timeout):
                ...
                # ***********   Q value update   ****************
                # Greedy method를 통해 현재 policy를 바탕으로 expected return이 가장 큰 action을 찾음
                max_action = np.argmax(self.Q_values[next_state[0], next_state[1]])
                # Constant alpha 방식으로 sample mean을 구하여 현재 state, action에서의 Q value를 update
                self.Q_values[state[0], state[1], action] += alpha * (reward + discount \
                * self.Q_values[next_state[0], next_state[1], max_action] \
                - self.Q_values[state[0], state[1], action])
                # Interaction을 통해 얻은 next state를 다음 iteration에서의 현재 state로 사용한다. 
                state = next_state
                # ************************************************
